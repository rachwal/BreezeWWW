$(function () {
    ...
    $("#add-component-save").click(function (e) {
        ...
        for (var i = 0; i < inPorts[0].length; i++) {
            inPortsModel.append("<input id=\"InPorts_" + i + "__Name\" name=\"InPorts[" + i + "].Name\" value=\"" + inPorts[0][i].value + "\" type=\"hidden\">");
        }
        var outPortsModel = $("#add-component-out-ports-model");
        for (var j = 0; j < outPorts[0].length; j++) {
            outPortsModel.append("<input id=\"OutPorts_" + j + "__Name\" name=\"OutPorts[" + j + "].Name\" value=\"" + outPorts[0][j].value+"\" type=\"hidden\">");
        }
        $("form").submit();
    });
});
